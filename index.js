...
(async () => {
  await utils.startMongoConnection().catch(reason => {
    console.log(reason);
    utils.stopMongoConnection();
    process.exitCode = 1;
  });
  ...
  slackInteractions.action({ callbackId: 'pick-a-dev' }, require('./actions').handleAction);
  slackInteractions.action({ callbackId: 'user_unavailable_selection' }, require('./actions').setUserUnavailable);
